...
def import_detailed_playbooks():
    """
    Import actual NDA policy rules from playbook.json into Redis for the Exercise 8 application
    """
    # Connect to Redis
    ...
    try:
        with open("data/policies/playbook.json", "r", encoding="utf-8") as f:
            playbook_data = json.load(f)
    except FileNotFoundError:
        print("[ERROR] data/policies/playbook.json file not found")
        print("Trying alternative path from exercise_8 root...")
        try:
            with open("D:/MyCode/AI/Victoria/lesson3/ai_agent_01/exercise_8/data/policies/playbook.json", "r", encoding="utf-8") as f:
                playbook_data = json.load(f)
        except FileNotFoundError:
            print("[ERROR] Could not find playbook.json at any path")
            return

    # Convert the detailed playbook data into a structure usable by the application
    # The application expects playbooks in the format {"name": "...", "rules": {...}}
    
    # Create a comprehensive NDA playbook from the detailed data
    nda_policy_rules = {}
    ...
    # Create the comprehensive NDA playbook
    comprehensive_nda_playbook = {
        "name": "Comprehensive NDA Policy Rules",
        "rules": nda_policy_rules
    }

    # Create a simplified version as well for comparison
    ...
    print("\n[SUCCESS] Detailed playbook import completed successfully!")

if __name__ == "__main__":
    import_detailed_playbooks()